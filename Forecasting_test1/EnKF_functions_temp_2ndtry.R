...
get_obs_matrix = function(obs_df, model_dates, n_step, n_states){
  ...
  obs_df_filtered = obs_df %>%
    dplyr::filter(as.Date(date) %in% model_dates) %>%
    mutate(date = as.Date(date)) %>%
    select(date, inflow_temp) %>%
...
kalman_filter = function(Y, R, obs, H, n_en, cur_step){
  ...
  Y_mean = matrix(mean(Y[ , cur_step, ]), nrow = length(Y[ , 1, 1]))
  ...
  for(i in 1:n_drivers){
    for(j in 1:n_step){
        drivers_out[j,i,] = rnorm(n = n_en,
                                ...
                                sd = abs(as.numeric(driver_cv[i] * drivers_filtered[j, driver_colnames[i]])))  }
  }
...
EnKF = function(n_en = 100,
                ...
                stop,
                time_step = 'days',
                obs_file = 'C:/Users/User/Desktop/Forecasting/EFI_aquatic_challenge_2021/Data/lake_c_data.rds',
                driver_file = 'C:/Users/User/Desktop/Forecasting/EFI_aquatic_challenge_2021/Data/lake_c_data.rds',
                n_states_est = 1,
                n_params_est = 0,
                n_params_obs = 0,
                obs_cv = 0.1,
 ...
 # decay_init = decay_init # Initial estimate of DOC decay rate day^-1
  ...
  temp_init = obs_df$inflow_temp[min(which(!is.na(obs_df$inflow_temp)))]

  state_cv = obs_cv #coefficient of variation of DOC observations
  state_sd = state_cv * temp_init
  init_cond_sd = init_cond_cv * temp_init

  param_cv = param_cv #coefficient of variation of DOC decay
  param_sd = param_cv * temp_init
  ...
  for(t in 2:n_step){
    for(n in 1:n_en){
      ...
      # run model;
      model_output = predict_inflow_temp(airtemp = drivers[t-1, 1, n],
                                      inflow_temp = Y[1, t-1, n])

      Y[1 , t, n] = model_output # store in Y vector
      #Y[1 , t, n] = model_output$temp_predict # store in Y vector
